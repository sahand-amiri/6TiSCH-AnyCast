...
class Tsch(object):
    ...
    def setIsSync(self,val):
        ...
        if self.isSync:
            ...
            self.mote.sf.handle_desynced_mote(self.mote)
            self.stopSendingEBs()
    ...
    def rxDone(self, packet, channel):
        ...
        if packet:
            ...
            # "packet": {"app": {"appcounter": 590, "timestamp": 164940}, 
                       # "mac": {"dstMac": "02-00-00-00-00-00-00-01", "pending_bit": false, "priority": false, "retriesLeft": 5, "seqnum": 55, "srcMac": "02-00-00-00-00-00-00-0D"}, 
                       # "net": {"downward": false, "dstIp": "fd00::1:0", "hop_limit": 64, "packet_length": 90, "srcIp": "fd00::d"}, 
                       # "type": "DATA"}
            if (
                    (packet[u'mac'][u'dstMac'] != d.BROADCAST_ADDRESS)
                    and
                    ...
                    (
                        ...
                        (
                            ...
                            ("mac3" not in packet)
                            and
                            (self.mote.is_my_mac_addr(packet[u'mac2'][u'dstMac']) is False)
                        )
                        or
                        (
                            ("mac2" in packet)
                            ...
                            (self.mote.is_my_mac_addr(packet[u'mac2'][u'dstMac']) is False)
                            and
                            (self.mote.is_my_mac_addr(packet[u'mac3'][u'dstMac']) is False)
                        )
                    )
                ):
                return False # isACKed
            ...
            if (
                    (self.mote.is_my_mac_addr(packet[u'mac'][u'dstMac']))
                    or
                    (
                        ("mac2" in packet)
                        and
                        (self.mote.is_my_mac_addr(packet[u'mac2'][u'dstMac']))
                    )
                    or
                    (
                        ("mac3" in packet)
                        and
                        (self.mote.is_my_mac_addr(packet[u'mac3'][u'dstMac']))
                    )
                ):
    ...
    def _schedule_next_active_slot(self):
        f = open("_action_active_cell.txt", "a")
        ...
        li=list()
        try:
            #tsDiffMin = min(#dar inja faghat slotframe shomare 1 unam faghat slot 95 esh cell dare
            #    [
            ...
            for _, slotframe in list(self.slotframes.items()):
                if (len(slotframe.get_busy_slots()) > 0):
                    li.append(slotframe.get_num_slots_to_next_active_cell(asn))
                f.write("ASN "+str(asn)+" |MOTE "+str(self.mote.id)+" |SLOTFRAME: "+str(_)+" (slotframe.get_busy_slots()): "+str(slotframe.get_busy_slots())+"\n")
            f.write("li "+str(li)+"\n")
            tsDiffMin=min(li)
            #print("tsDiffMin: ----------------------- >"+str(tsDiffMin))
        except ValueError:
        ...
        # print(str(self.slotframes.items()))
        self.engine.scheduleAtAsn(
            asn            = asn+tsDiffMin,
        ...
        f = open("_action_active_cell.txt", "a")
        f.write("MOTE: "+str(self.mote.id)+" _schedule_next_active_slot in ASN "+str(asn)+" for next asn: "+str(asn+tsDiffMin)+"\n")
        f.write("in asn "+str(asn)+" slotframes: \n")
        for key,value in self.slotframes.items():
    ...
    def _action_active_cell(self):
        ...
        candidate_cells = []
        for _, slotframe in list(self.slotframes.items()):#slotframes: {0: slotframe(length: 101, num_cells: 0)
                                                                     #, 1: slotframe(length: 101, num_cells: 1)
                                                                     #, 2: slotframe(length: 101, num_cells: 0)}
            candidate_cells = slotframe.get_cells_at_asn(asn)#asn=95
            #print("in asn: "+str(asn)+"\ncandidate_cells: "+str(candidate_cells))
        ...
        if self.active_cell:
            if self.pktToSend is None:
                ...
                if self.pktToSend[u'mac'][u'dstMac'] == self.clock.source:#IN VAGHTI EB DARYAFT MISHE AZ HARKI DARYAFT BESHE HAMOON TO sync() class
                                                                          # Clock() mishe self.source
        ...
        f = open("_action_active_cell.txt", "a")
        f.write("in asn "+str(asn)+" candidate_cells :\n"+str(candidate_cells)+"\n")
        f.write("in asn "+str(asn)+" self.pktToSend :\n"+str(self.pktToSend)+"\n")
        f.write("in asn "+str(asn)+" active_cell :\n"+str(self.active_cell)+"\n")
        f.write("in asn "+str(asn)+" self.txQueue :\n"+str(self.txQueue)+"\n---------------------------------------------------------------------\n")
