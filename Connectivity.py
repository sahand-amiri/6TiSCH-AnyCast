...
class Connectivity(object):
    ...
    def __init__(self, sim_engine=None):
        ...
        conn_class_name = self.settings.conn_class#conn_class=Random|Linear|FulluMeshed
        ...
        self.matrix = matrix_class(self)#ConnectivityMatrixRandom
    ...
    def propagate(self):
        ...
        for mote in self.engine.motes:
            ...
            # get all receivers
                ...
                pass
        #+"\nthisTran: "+str(thisTran)
        f = open("connectivity.txt", "a")
        f.write("asn "+str(asn)+" :\n - receivers_by_channel: "+str(receivers_by_channel)+"\n - transmissions_by_channel: "+str(transmissions_by_channel)+"\n")
        ...
        for channel in set(receivers_by_channel.keys()) - set(transmissions_by_channel.keys()):
            ...
            for listener_id in receivers_by_channel[channel]:
                ...
                f.write(" - sentAck = "+str(sentAck)+" for mote: "+str(self.engine.motes[listener_id].id)+"\n")
        #f = open("connectivity.txt", "a")
        #f.write("asn "+str(asn)+"\nreceivers_by_channel: "+str(receivers_by_channel)+"\ntransmissions_by_channel: "+str(transmissions_by_channel)+"\n")
        ...
        f.write(" - transmissions_by_channel join receivers_by_channel: "+str(set(transmissions_by_channel.keys()) & set(receivers_by_channel.keys()))+"\n")
            ...
            for listener_id in receivers_by_channel[channel]:
                ...
                if len(transmissions_by_channel[channel]) > 1:
                    for t in transmissions_by_channel[channel]:
                        ...
                        f.write("\n"+str(t['tx_mote_id'])+": peamble_pdr-> "+str(peamble_pdr)+" random_value-> "+str(random_value))
                        ...
                        f.write("\ninterfering_transmissions before if t[u'txTime'] < lockon_transmission[u'txTime']: "+str(interfering_transmissions))
                        f.write("\nlockon_transmission before if t[u'txTime'] < lockon_transmission[u'txTime']: "+str(lockon_transmission))
                        ...
                        f.write("\ninterfering_transmissions after if t[u'txTime'] < lockon_transmission[u'txTime']: "+str(interfering_transmissions))
                        f.write("\nlockon_transmission after if t[u'txTime'] < lockon_transmission[u'txTime']: "+str(lockon_transmission))
                ...
                f.write("\nsrc: "+str(lockon_transmission[u'tx_mote_id'])+"\ndst: "+str(listener_id)+"\nlockon_random_value : "+str(lockon_random_value)+" \npacket_pdr: "+str(packet_pdr)+"\n")
                ...
                if lockon_random_value < packet_pdr:
                    ...
                    f.write("receivedAck: "+str(receivedAck)+"\n")
                    ...
                    if receivedAck and self.settings.conn_simulate_ack_drop:
                        ...
                        f.write("receivedAck1: "+str(receivedAck)+"\n")
                    if receivedAck:
                        # keep track of the number of ACKs received by
                        # that transmission
                        if lockon_transmission[u'numACKs'] == 1:
                        ...
                        pass
                    f.write("lockon_transmission[u'numACKs']: "+str(lockon_transmission[u'numACKs'])+"\n")
                else:
                    ...
                    ###################### about packet dropped per link ###########################################################
                    motes = self.engine.motes
                    root = motes[0]
                    sender_mote = self.engine.get_mote_by_id(lockon_transmission[u'tx_mote_id'])
                    receiver_mote = self.engine.get_mote_by_id(listener_id)
                    if lockon_transmission['packet']["type"] == "DATA" and receiver_mote.get_mac_addr() == sender_mote.rpl.getPreferredParent():
                        packetDroppedPerLink = root.sf.packetDroppedPerLink
                        if lockon_transmission[u'tx_mote_id'] in packetDroppedPerLink:
                            if listener_id in packetDroppedPerLink[lockon_transmission[u'tx_mote_id']]:
                                packetDroppedPerLink[lockon_transmission[u'tx_mote_id']][listener_id] += 1
                            else:
                                packetDroppedPerLink[lockon_transmission[u'tx_mote_id']][listener_id] = 1
                        else:
                            packetDroppedPerLink[lockon_transmission[u'tx_mote_id']] = {}
                            packetDroppedPerLink[lockon_transmission[u'tx_mote_id']][listener_id] = 1
                    ##################### about handle packet by secondary or third parent ########################################
                    tree = root.rpl.tree
                    ...
                    if packet["type"] == "DATA":
                        if sender_mote.id in otherParents:
                            retriesLeft = packet["mac"]["retriesLeft"]
                            appcounter = packet["app"]["appcounter"]
                            if receiver_mote.get_mac_addr() == sender_mote.rpl.getPreferredParent():
                                print(receiver_mote.id," unreceive1 packet appcounter ",appcounter)
                                parent_id = otherParents[sender_mote.id][0]
                                secondary_parent = self.engine.get_mote_by_id(parent_id)
                                self.add_packet_to_unreceived_packets(packet, sender_mote, receiver_mote)
                                print(receiver_mote.id," add to unreceive1 packet appcounter ",appcounter)
                                third_parent = None
                                if "mac3" in packet:
                                    third_parent_mac = packet[u'mac3'][u'dstMac']
                                    third_parent = self.engine.get_mote_by_mac_addr(third_parent_mac)
                                self.check_packet_in_second_parent_waiting_list(secondary_parent,third_parent,packet,sender_mote,receiver_mote,retriesLeft)
                            elif "mac2" in packet and receiver_mote.get_mac_addr() == packet[u'mac2'][u'dstMac']:
                                print(receiver_mote.id," unreceive2 packet appcounter ",appcounter)
                                parent_id = otherParents[sender_mote.id][1]
                                third_parent = self.engine.get_mote_by_id(parent_id)
                                self.add_packet_to_unreceived_packets(packet, sender_mote, receiver_mote)
                                print(receiver_mote.id," add to unreceive2 packet appcounter ",appcounter)
                                self.check_packet_in_other_parent_list_from_secondary(packet,sender_mote,receiver_mote)
                            elif "mac3" in packet and receiver_mote.get_mac_addr() == packet[u'mac3'][u'dstMac']:
                                print(receiver_mote.id," unreceive3 packet appcounter ",appcounter)
                                self.add_packet_to_unreceived_packets(packet, sender_mote, receiver_mote)
                                print(receiver_mote.id," add to unreceive3 packet appcounter ",appcounter)
                # done processing this listener
            #inja miaim hameye un packet hai ke dar waiting list ha baghi moonde ro ersal mikonim
            motes = self.engine.motes
            root = motes[0]
            otherParents = root.rpl.otherParents
            receiver_list = receivers_by_channel[channel]
            for t in transmissions_by_channel[channel]:
                packet = copy.deepcopy(t['packet'])
                if packet['type'] == 'DATA':
                    if 'mac2' in packet:
                        preferred_parent = self.engine.get_mote_by_mac_addr(packet['mac']['dstMac'])
                        secondary_parent = self.engine.get_mote_by_mac_addr(packet['mac2']['dstMac'])
                        if secondary_parent.id in receiver_list:
                            sender_id = t['tx_mote_id']
                            sender_mote = self.engine.get_mote_by_id(sender_id)
                            retriesLeft = packet['mac']['retriesLeft']
                            appcounter = packet['app']['appcounter']
                            if sender_id in secondary_parent.sixlowpan.waiting_packets:
                                if appcounter in secondary_parent.sixlowpan.waiting_packets[sender_id]:
                                    if retriesLeft in secondary_parent.sixlowpan.waiting_packets[sender_id][appcounter]:
                                        if secondary_parent.sixlowpan.waiting_packets[sender_id][appcounter][retriesLeft] == 1:
                                            secondary_parent.sixlowpan.waiting_packets[sender_id][appcounter][retriesLeft] = 0
                                            secondary_parent.sixlowpan.add_packet_to_received_packets(packet,sender_mote)
                                            secondary_parent.sixlowpan.forward(packet)
                                            secondary_parent.sixlowpan.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                                            print(preferred_parent.id," not in receiver list so ",secondary_parent.id," forward packet appcounter ",appcounter," after connectivity")
                                            continue
                        if "mac3" in packet:
                            third_parent = self.engine.get_mote_by_mac_addr(packet['mac3']['dstMac'])
                            if third_parent.id in receiver_list:
                                sender_id = t['tx_mote_id']
                                sender_mote = self.engine.get_mote_by_id(sender_id)
                                retriesLeft = packet['mac']['retriesLeft']
                                appcounter = packet['app']['appcounter']
                                if sender_id in third_parent.sixlowpan.waiting_packets:
                                    if appcounter in third_parent.sixlowpan.waiting_packets[sender_id]:
                                        if retriesLeft in third_parent.sixlowpan.waiting_packets[sender_id][appcounter]:    
                                            if third_parent.sixlowpan.waiting_packets[sender_id][appcounter][retriesLeft] == 1:
                                                third_parent.sixlowpan.waiting_packets[sender_id][appcounter][retriesLeft] = 0
                                                third_parent.sixlowpan.add_packet_to_received_packets(packet,sender_mote)
                                                third_parent.sixlowpan.forward(packet)
                                                third_parent.sixlowpan.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                                                print(preferred_parent.id," not in receiver list and secondary parent also, so ",third_parent.id," forward packet appcounter ",appcounter," after connectivity")
    ...
    def add_packet_to_unreceived_packets(self, packet, sender_mote, receiver_mote):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        if sender_mote.id in receiver_mote.sixlowpan.received_packets:
            if appcounter in receiver_mote.sixlowpan.received_packets[sender_mote.id]:
                receiver_mote.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] = 0
            else:
                receiver_mote.sixlowpan.received_packets[sender_mote.id][appcounter] = {}
                receiver_mote.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] = 0
        else:
            receiver_mote.sixlowpan.received_packets[sender_mote.id] = {}
            receiver_mote.sixlowpan.received_packets[sender_mote.id][appcounter] = {}
            receiver_mote.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] = 0
    
    def check_packet_in_other_parent_waiting_list(self, otherParent, packet, sender_mote, receiver_mote, retriesLeft):
        appcounter = packet["app"]["appcounter"]
        packet = copy.deepcopy(packet)
        if sender_mote.id in otherParent.sixlowpan.waiting_packets:
            if appcounter in otherParent.sixlowpan.waiting_packets[sender_mote.id]:
                if retriesLeft in otherParent.sixlowpan.waiting_packets[sender_mote.id][appcounter]:
                    if otherParent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                        otherParent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 0
                        otherParent.sixlowpan.add_packet_to_received_packets(packet,sender_mote)
                        otherParent.sixlowpan.forward(packet)
                        otherParent.sixlowpan.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                        print(receiver_mote.id," cant receive packet so ",otherParent.id," forward packet appcounter ",appcounter)
    
    def check_packet_in_second_parent_waiting_list(self, secondParent, thirdParent, packet, sender_mote, receiver_mote, retriesLeft):
        appcounter = packet["app"]["appcounter"]
        if sender_mote.id in secondParent.sixlowpan.waiting_packets:
            if appcounter in secondParent.sixlowpan.waiting_packets[sender_mote.id]:
                if retriesLeft in secondParent.sixlowpan.waiting_packets[sender_mote.id][appcounter]:
                    if secondParent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                        secondParent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 0
                        secondParent.sixlowpan.add_packet_to_received_packets(packet,sender_mote)
                        secondParent.sixlowpan.forward(packet)
                        secondParent.sixlowpan.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                        print(receiver_mote.id," cant receive packet so ",secondParent.id," forward packet appcounter ",appcounter)
                else:
                    self.check_packet_in_second_parent_received_list(secondParent,thirdParent,packet,sender_mote,receiver_mote,retriesLeft)
            else:
                self.check_packet_in_second_parent_received_list(secondParent,thirdParent,packet,sender_mote,receiver_mote,retriesLeft)
        else:
            self.check_packet_in_second_parent_received_list(secondParent,thirdParent,packet,sender_mote,receiver_mote,retriesLeft)

    def check_packet_in_second_parent_received_list(self, secondParent, thirdParent, packet, sender_mote, receiver_mote, retriesLeft):
        if thirdParent != None:
            appcounter = packet["app"]["appcounter"]
            if sender_mote.id in secondParent.sixlowpan.received_packets:
                if appcounter in secondParent.sixlowpan.received_packets[sender_mote.id]:
                    if retriesLeft in secondParent.sixlowpan.received_packets[sender_mote.id][appcounter]:
                        if secondParent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 0:
                            self.check_packet_in_third_parent_waiting_list(thirdParent,packet,sender_mote,receiver_mote,retriesLeft)
        else:
            print("mote "+str(sender_mote.id)+" has no third parent and secondary parent has not received packet yet")
    
    def check_packet_in_third_parent_waiting_list(self, thirdParent, packet, sender_mote, receiver_mote, retriesLeft):
        appcounter = packet["app"]["appcounter"]
        if sender_mote.id in thirdParent.sixlowpan.waiting_packets:
            if appcounter in thirdParent.sixlowpan.waiting_packets[sender_mote.id]:
                if retriesLeft in thirdParent.sixlowpan.waiting_packets[sender_mote.id][appcounter]:
                    if thirdParent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                        thirdParent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 0
                        thirdParent.sixlowpan.add_packet_to_received_packets(packet,sender_mote)
                        thirdParent.sixlowpan.forward(packet)
                        thirdParent.sixlowpan.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                        print(receiver_mote.id," cant receive packet so ",thirdParent.id," forward packet appcounter ",appcounter)    
    
    def check_packet_in_other_parent_list_from_secondary(self, packet, sender_mote, receiver_mote):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        primary_parent_mac = packet["mac"]["dstMac"]
        primary_parent = self.engine.get_mote_by_mac_addr(primary_parent_mac)
        third_parent = None
        if "mac3" in packet:
            third_parent_mac = packet[u'mac3'][u'dstMac']
            third_parent = self.engine.get_mote_by_mac_addr(third_parent_mac)
        if sender_mote.id in primary_parent.sixlowpan.received_packets:
            if appcounter in primary_parent.sixlowpan.received_packets[sender_mote.id]:
                if retriesLeft in primary_parent.sixlowpan.received_packets[sender_mote.id][appcounter]:
                    if primary_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 0:
                        self.check_packet_in_third_parent_waiting_list(third_parent,packet,sender_mote,receiver_mote,retriesLeft)
    ...
    def _schedule_propagate(self):
        ...
        self.engine.scheduleAtAsn(
            ...
            intraSlotOrder   = d.INTRASLOTORDER_PROPAGATE,#1
        )
...
class ConnectivityMatrixBase(object):
    ...
    def __init__(self, connectivity):
        ...
        f=open("Connectivity_init.txt",'w')
        f.write(str(self._matrix))
        #print(self._matrix)
