...
################################################################################################################################
    ...
    def DCAB(self, old_preferred=None, new_preferred=None):
        ...
        if new_preferred:
            parent_mote = self.engine.get_mote_by_mac_addr(new_preferred)
            child_mote = self.mote
            tree[child_mote.id] = parent_mote.id
        elif new_preferred == None:
            child_mote = self.mote
            if child_mote.id in tree:
                del tree[child_mote.id]
        ...
        for slot in schedulingMx:
            for channel in schedulingMx[slot]:
                ...
                if sender_id in otherParents:
                    if old_preferred != None:
                        if new_preferred == "02-00-00-00-00-01-00-00":
                            for parent_id in otherParents[sender_id]:
                        ...
                        elif new_preferred != "02-00-00-00-00-01-00-00":
                            child_mote = self.engine.get_mote_by_id(sender_id)
                            parent_mote = self.engine.get_mote_by_mac_addr(new_preferred)
                            if new_preferred != child_mote.rpl.getPreferredParent():
                                for parent_id in otherParents[sender_id]:
                                    receiver_mote = self.engine.get_mote_by_id(parent_id)
                                    sender_mote.tsch.deleteCell(
                                        slotOffset       = slot,
        ...
        schedulingMx.clear()
        root.sf.schedulingMatrix = self.create_scheduling_matrix(treeDict,schedulingMx)
    ...
    def create_scheduling_matrix(self,treeDict,schedulingMatrix):
        for routes in treeDict:
            for route in treeDict[routes]:
                slot = 1
                for link in route:
    ...
    def delete_old_other_parents(self,joined_mote,otherParents):
        motes = self.engine.motes
        root = motes[0]
        schedulingMx = root.sf.schedulingMatrix
        for slot in schedulingMx:
                ...
                if sender_id == desynced_mote.id:
                    ...
                    if sender_id in otherParents:
                        for parent_id in otherParents[sender_id]:
                            ...
                            print("delete(from desynced-other parent) cells: [",slot,channel,"] for mote ",sender_id,"->",receiver_mote.id)
                        del otherParents[sender_id]
                    break
