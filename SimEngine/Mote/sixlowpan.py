...
class Sixlowpan(object):
    ...
    def __init__(self, mote):
        ...
        self.waiting_packets = {} # packets waiting in this list
        self.received_packets = {} # {sender_mote_id:{appcounter:1},sender_mote_id:{appcounter:0}} 
    ...
    def sendPacket(self, packet):
        ...
        if goOn:
            ...
            root = motes[0]
            tree = root.rpl.tree
            if packet[u'type'] == d.PKT_TYPE_DATA and self.mote.id in root.rpl.otherParents:
                packet[u'mac2'] = {
                    u'srcMac': self.mote.get_mac_addr(),
                    u'dstMac': self.engine.get_mote_by_id(root.rpl.otherParents[self.mote.id][0]).get_mac_addr()
                }
                if len(root.rpl.otherParents[self.mote.id]) == 2:
                    packet[u'mac3'] = {
                        u'srcMac': self.mote.get_mac_addr(),
                        u'dstMac': self.engine.get_mote_by_id(root.rpl.otherParents[self.mote.id][1]).get_mac_addr()
                    }
                print(self.mote.id," send packet appcounter ",packet['app']['appcounter'])
    ...
    def recvPacket(self, packet):
        ...
        if goOn:
            if  (
                ...
                sender_mote = self.engine.get_mote_by_mac_addr(packet[u'mac'][u'srcMac'])
                if packet["type"] == "DATA":
                    ...
                    if sender_mote.id in otherParents:
                        if self.mote.is_my_mac_addr(packet[u'mac'][u'dstMac']):
                            self.forward(packet)#inja ino aval miarim chon drop packet kole packet ro pak mikone
                            self.add_packet_to_received_packets(packet, sender_mote)
                            self.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                            print(self.mote.id," forward1 packet appcounter ",appcounter)
                        elif self.mote.is_my_mac_addr(packet[u'mac2'][u'dstMac']):
                            print(self.mote.id," receive2 packet appcounter ",appcounter)
                            preferred_parent = self.engine.get_mote_by_mac_addr(packet[u'mac'][u'dstMac'])
                            self.check_in_secondary_received_packet_in_primary_parent(preferred_parent, packet, sender_mote)
                        elif "mac3" in packet and self.mote.is_my_mac_addr(packet[u'mac3'][u'dstMac']):
                            print(self.mote.id," receive3 packet appcounter ",appcounter)
                            preferred_parent = self.engine.get_mote_by_mac_addr(packet[u'mac'][u'dstMac'])
                            secondary_parent = self.engine.get_mote_by_mac_addr(packet[u'mac2'][u'dstMac'])
                            self.check_in_third_received_packet_in_primary_secondary_parent(preferred_parent, secondary_parent, packet, sender_mote)
                    else:
                        self.forward(packet)                
                else:
                    self.forward(packet)
    
    def add_packet_to_waiting_packets(self, packet, sender_mote, secondary_or_third_parent):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        if sender_mote.id in self.waiting_packets:
            if appcounter in self.waiting_packets[sender_mote.id]:
                self.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 1
            else:
                self.waiting_packets[sender_mote.id][appcounter] = {}
                self.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 1
        else:
            self.waiting_packets[sender_mote.id] = {}
            self.waiting_packets[sender_mote.id][appcounter] = {}
            self.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 1
        if secondary_or_third_parent==2:
            self.log(
                    SimEngine.SimLog.LOG_PACKET_IN_WAITING_BUCKET2,
                    {
                        u'_mote_id':  self.mote.id,
                        u'packet':    packet,
                        u'reason':    SimEngine.SimLog.WAITINGREASON_ANYCAST,
                    }
                )
        elif secondary_or_third_parent==3:
            self.log(
                    SimEngine.SimLog.LOG_PACKET_IN_WAITING_BUCKET3,
                    {
                        u'_mote_id':  self.mote.id,
                        u'packet':    packet,
                        u'reason':    SimEngine.SimLog.WAITINGREASON_ANYCAST,
    ...
    def add_packet_to_received_packets(self, packet, sender_mote):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        if sender_mote.id in self.received_packets:
            if appcounter in self.received_packets[sender_mote.id]:
                self.received_packets[sender_mote.id][appcounter][retriesLeft] = 1
            else:
                self.received_packets[sender_mote.id][appcounter] = {}
                self.received_packets[sender_mote.id][appcounter][retriesLeft] = 1
        else:
            self.received_packets[sender_mote.id] = {}
            self.received_packets[sender_mote.id][appcounter] = {}
            self.received_packets[sender_mote.id][appcounter][retriesLeft] = 1
    
    def delete_packet_from_other_parent_waiting_packets_list(self, packet, sender_mote):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        motes = self.engine.motes
        root = motes[0]
        tree = root.rpl.tree
        otherParents = root.rpl.otherParents
        for parent_id in otherParents[sender_mote.id]:
            another_parent = self.engine.get_mote_by_id(parent_id)
            if sender_mote.id in another_parent.sixlowpan.waiting_packets:
                if appcounter in another_parent.sixlowpan.waiting_packets[sender_mote.id]:
                    if retriesLeft in another_parent.sixlowpan.waiting_packets[sender_mote.id][appcounter]:
                        if another_parent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                            another_parent.sixlowpan.waiting_packets[sender_mote.id][appcounter][retriesLeft] = 0
                            another_parent.drop_packet(
                                    packet = packet,
                                    reason = SimEngine.SimLog.DROPREASON_ANYCAST
                                )
                            print(another_parent.id," delete packet from waiting appcounter ",appcounter)
    
    def check_in_secondary_received_packet_in_primary_parent(self, preferred_parent, packet, sender_mote):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        if sender_mote.id in preferred_parent.sixlowpan.received_packets:
            if appcounter in preferred_parent.sixlowpan.received_packets[sender_mote.id]:
                if retriesLeft in preferred_parent.sixlowpan.received_packets[sender_mote.id][appcounter]:
                    if preferred_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                        self.mote.drop_packet(
                                packet = packet,
                                reason = SimEngine.SimLog.DROPREASON_ANYCAST
                            )
                        print(self.mote.id," drop2 packet appcounter ",appcounter)
                    elif preferred_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 0:
                        self.add_packet_to_received_packets(packet, sender_mote)
                        self.forward(packet)
                        self.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                        print(self.mote.id," forward2 packet appcounter ",appcounter)
                else:
                    self.add_packet_to_waiting_packets(packet, sender_mote, 2)
                    print(self.mote.id," waiting2-1 packet appcounter ",appcounter)
            else:
                self.add_packet_to_waiting_packets(packet, sender_mote, 2)
                print(self.mote.id," waiting2-2 packet appcounter ",appcounter)
        else:
            self.add_packet_to_waiting_packets(packet, sender_mote, 2)
            print(self.mote.id," waiting2-3 packet appcounter ",appcounter)
    
    def check_in_third_received_packet_in_primary_secondary_parent(self, preferred_parent, secondary_parent, packet, sender_mote):
        appcounter = packet["app"]["appcounter"]
        retriesLeft = packet["mac"]["retriesLeft"]
        if sender_mote.id in preferred_parent.sixlowpan.received_packets:
            if appcounter in preferred_parent.sixlowpan.received_packets[sender_mote.id]:
                if retriesLeft in preferred_parent.sixlowpan.received_packets[sender_mote.id][appcounter]:
                    if preferred_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                        ...
                        print(self.mote.id," drop3-1 packet appcounter ",appcounter)
                    elif preferred_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 0:
                        if sender_mote.id in secondary_parent.sixlowpan.received_packets:
                            if appcounter in secondary_parent.sixlowpan.received_packets[sender_mote.id]:
                                if retriesLeft in secondary_parent.sixlowpan.received_packets[sender_mote.id][appcounter]:
                                    if secondary_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 1:
                                        self.mote.drop_packet(
                                                packet = packet,
                                                reason = SimEngine.SimLog.DROPREASON_ANYCAST
                                            )
                                        print(self.mote.id," drop3-2 packet appcounter ",appcounter)
                                    elif secondary_parent.sixlowpan.received_packets[sender_mote.id][appcounter][retriesLeft] == 0:
                                        self.add_packet_to_received_packets(packet, sender_mote)
                                        self.forward(packet)
                                        self.delete_packet_from_other_parent_waiting_packets_list(packet, sender_mote)
                                        print(self.mote.id," forward3 packet appcounter ",appcounter)
                                else:
                                    self.add_packet_to_waiting_packets(packet, sender_mote, 3)
                                    print(self.mote.id," waiting3-1 packet appcounter ",appcounter)
                            else:
                                self.add_packet_to_waiting_packets(packet, sender_mote, 3)
                                print(self.mote.id," waiting3-2 packet appcounter ",appcounter)
                        else:
                            self.add_packet_to_waiting_packets(packet, sender_mote, 3)
                            print(self.mote.id," waiting3-3 packet appcounter ",appcounter)
                else:
                    self.add_packet_to_waiting_packets(packet, sender_mote, 2)
                    print(self.mote.id," waiting3-4 packet appcounter ",appcounter)
            else:
                self.add_packet_to_waiting_packets(packet, sender_mote, 2)
                print(self.mote.id," waiting3-5 packet appcounter ",appcounter)
        else:
            self.add_packet_to_waiting_packets(packet, sender_mote, 2)
            print(self.mote.id," waiting3-6 packet appcounter ",appcounter)
        ...
        if (dst_ip_addr.words[0] & 0xFF00) == 0xFF00:
            ...
            if self.mote.rpl.dodagId is None:
                ...
                mac_addr = str(self.mote.tsch.join_proxy)
                    ...
                    (
                        ((src_ip_addr.words[0] & 0xFE80) == 0xFE80)#inja check mikone aya ip link local ast ya global?
                    )
                    or
                    (
