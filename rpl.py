...
class Rpl(object):
    ...
    def determine_other_parents(self, new_parent, old_parent, joined_mote):
        # {
            # 1:0,1:1
            # 2:0,2:1
            # 3:0,3:1
            ...
            # 5:8.5:3
        # }
        print("self.otherParents before handle")
        print(self.otherParents)
        #first delete cells between joined mote and old secondary and third parents
        if old_parent != None:
            self.mote.sf.delete_old_parents(joined_mote,self.otherParents,self.tree)
        #then add new secondary and third parents
        if new_parent != None:
            parent_mote = self.engine.get_mote_by_mac_addr(new_parent)
            self.tree[joined_mote.id] = parent_mote.id
            if new_parent != "02-00-00-00-00-01-00-00":
                connectivity = self.engine.connectivity
                mote_level = {}
                for child in self.tree:
                    mote_level[child] = 0
                    _child = child
                    while _child in self.tree:
                        mote_level[child] += 1
                        _child = self.tree[_child]
                mote_level[0] = 0
                
                parent_level = mote_level[parent_mote.id]
                for parent in mote_level:
                    if parent == parent_mote.id:
                        del mote_level[parent]
                        break
                available_parent_by_level = []
                for mote_id in mote_level:
                    if mote_level[mote_id] <= parent_level:
                        available_parent_by_level.append(mote_id)
                        
                mote_children = {}
                parents = list(self.tree.values())
                for child in self.tree:
                    mote_children[child] = 0
                    for item in parents:
                        if item == child:
                            mote_children[child] += 1
                other_parents = list(self.otherParents.values()) 
                for child in self.tree:
                    for items in other_parents:
                        if child in items:
                            mote_children[child] += 1
                mote_children[0] = 0
                ...
                for items in other_parents:
                    if 0 in items:
                        mote_children[0] += 1
                for parent in mote_children:
                    if parent == parent_mote.id:
                        del mote_children[parent]
                        break
                print("mote_children")
                print(mote_children)
                        
                # unsorted = {k:mote_children[k] for k in available_parent_by_level}
                # available_parent = [k for k,v in (sorted(unsorted.items(), key=operator.itemgetter(1)))]
                parents_by_pdr = {}
                for mote_id in available_parent_by_level:
                    receiver_id = mote_id
                    sender_id = joined_mote.id
                    parents_by_pdr[receiver_id] = numpy.mean([
                    connectivity.get_pdr(src_id, dst_id, channel)
                ...
                available_parent = [k for k,v in (sorted(parents_by_pdr.items(), key=operator.itemgetter(1),reverse=True))]
                
                st_parent = []
                for parent in available_parent:
                    if parent in mote_children:
                        if mote_children[parent] < 3:
                            st_parent.append(parent)
                if len(st_parent) < 2:
                    if len(st_parent) < 1:
                        print("mote ",joined_mote.id,"has noe other parents")
                        if joined_mote.id in self.otherParents:
                            del self.otherParents[joined_mote.id]
                    else:
                        print("mote ",joined_mote.id,"has just secondary parent",st_parent[0])
                        self.otherParents[joined_mote.id] = st_parent
                else:
                    st_parent = st_parent[0:2]
                    print("mote ",joined_mote.id," has secondary parent ",st_parent[0]," and third parents ",st_parent[1])
                    self.otherParents[joined_mote.id] = st_parent
                
        print("self.otherParents after handle")
        print(self.otherParents)
    ...
    def indicate_preferred_parent_change(self, old_preferred, new_preferred):
        ...
        root = motes[0]
        root.rpl.determine_other_parents(new_preferred, old_preferred, self.mote)
        self.mote.sf.DCAB(old_preferred=old_preferred, new_preferred=new_preferred)
...
class RplOFBestLinkPDR(RplOF0):
    ACCEPTABLE_LOWEST_PDR  = 1.0 / RplOF0.UPPER_LIMIT_OF_ACCEPTABLE_ETX#3
